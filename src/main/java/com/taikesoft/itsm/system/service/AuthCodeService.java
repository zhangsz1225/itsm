...
public class AuthCodeService {
	...
	private SysAuthCode dao = new SysAuthCode().dao();

	/**
	 * 创建注册激活授权码，一个小时后过期，3600 秒
	 */
	public String createRegActivateAuthCode(int accountId) {
		return createAuthCode(accountId, SysAuthCode.TYPE_REG_ACTIVATE, 3600);
	}
	...
	public String createRetrievePasswordAuthCode(int accountId) {
		return createAuthCode(accountId, SysAuthCode.TYPE_RETRIEVE_PASSWORD, 3600);
	}

	/**
	 * 获取授权码，授权码只能使用一次，在被获取后会被立即删除
	 */
	public SysAuthCode getAuthCode(String authCodeId) {
		if (StrKit.notBlank(authCodeId)) {
			SysAuthCode authCode = dao.findById(authCodeId.trim());    // authCode 是唯一的
			if (authCode != null) {
	...
	private String createAuthCode(int accountId, int authType, int expireTime) {
		long expireAt = System.currentTimeMillis() + ((long) expireTime * 1000);

		SysAuthCode ac = new  SysAuthCode();
		...
		if (ac.save()) {
			...
			throw new RuntimeException("保存 sys_auth_code 记录失败，请联系管理员");
		}
	}
	...
	public Ret peekAuthCode(String id) {
		SysAuthCode authCode = dao.findById(id);
		if (authCode != null) {
	...
	public int clearExpiredAuthCode() {
		return Db.update("delete from sys_auth_code where expireAt < ?", System.currentTimeMillis());
	}
}
